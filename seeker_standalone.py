...
class Config:
    ...
    status_check_interval: int = 3  # Interval in seconds for checking server status
    ...
    def __post_init__(self):
        ...
        if self.cnc_server_url is None:
            self.cnc_server_url = os.environ.get('CNC_SERVER_URL', 'http://localhost:7898')
...
class NodeRunner:
    ...
    def submit_batch(self, batch: List[str]):
        ...
        if result.get("error") == "STOP":
            logger.warning("Received STOP signal from server")
            self.stop_processes()
            ...
            logger.info(f"Successfully posted batch of {len(batch)} items")
    ...
    def run(self, filename: str) -> Optional[str]:
        ...
        try:
            executable_path = self.setup_executable(filename)
...
def main():
    ...
    try:
        ...
        search_prefix = sys.argv[1] if len(sys.argv) > 1 else client.get_search_prefix()

        if len(search_prefix) < 2:
            logger.error("Incorrect prefix format")
            sys.exit(1)
        ...
        runner_thread = threading.Thread(target=runner.run, args=("Seeker_CUDA",))
        runner_thread.start()

        # Main loop for status checking
        while runner_thread.is_alive():
            current_time = time.time()
            if current_time - last_check_time >= config.status_check_interval:
        ...
        runner_thread.join()
        if result := runner.run("Seeker_CUDA"):
            logger.info(f"Runner completed with message: {result}")
