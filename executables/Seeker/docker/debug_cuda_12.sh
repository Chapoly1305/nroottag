...
docker run --rm -v "$(pwd):/app" -w /app nvidia/cuda:${CUDA}-devel-ubuntu22.04 \
    bash -c "make clean && make debug=1 CUDA=/usr/local/cuda CXXCUDA=/usr/bin/g++ gpu=1 CCAP=${CCAP} all && \
             cp Seeker /app/executables/Seeker_CUDA && chmod 777 /app/executables/*"
